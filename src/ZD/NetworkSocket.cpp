...
namespace ZD
{
  ...
  std::shared_ptr<NetworkSocket> NetworkSocket::server(
    SocketType type, int port)
  {
    ...
    if (bind(fd, (struct sockaddr *)&addr, sizeof(addr)) < 0)
    {
      perror("server bind");
      fprintf(
        stderr, "Socket bind error (fd=%d type=%d port=%d)\n", fd, t, port);
      return {};
    }
        ...
        if (ret <= 0)
        {
          ...
          if (is_server())
          {
            ...
            if (client_fd > 0)
            {
              ...
              inet_ntop(
                addr.sin_family, &addr.sin_addr, oip_cstr, sizeof(oip_cstr));
