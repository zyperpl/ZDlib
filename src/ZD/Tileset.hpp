...
namespace ZD
{
  ...
  struct TileIndexRange
  {
    ...
    {
      if (idx.x >= x1 && idx.x <= x2 && idx.y >= y1 && idx.y <= y2)
        return true;
  ...
  public:
    ...
    void insert(Tile tile)
    {
      ...
      if (tiles.find(key(x, y)) != tiles.end())
      {
        remove(x, y);
      }
    ...
    const std::unordered_map<int64_t, Tile> get_tiles() const { return tiles; }

  protected:
    TilesMask redraw_mask;

    void set_redraw(int32_t x, int32_t y) { redraw_mask.set_minmax(x, y); }
  ...
  public:
    TilesetImageWrapper(
      std::shared_ptr<Tileset> tileset, std::shared_ptr<Tilemap> tilemap)
    : tileset { tileset }
    , tilemap { tilemap }
    {
    }
  ...
  private:
    ...
    int image_height()
    {
      return tileset->get_tile_height() * V_TILES_PER_IMAGE;
    }
