...
public:
  ...
  Painter(std::shared_ptr<uint32_t[]> data, const Size &size, PixelFormat::Type format);
  
  void set_pixel(const int x, const int y, const Color &color);
  void draw_image(const int x, const int y, const Image &image);
  void draw_image(const int x, const int y, const Image &image, double scale_x, double scale_y);
  void draw_image(const int x, const int y, const Image &image, const int width, const int height);
