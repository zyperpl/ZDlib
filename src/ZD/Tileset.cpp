...
Tileset::Tileset(std::shared_ptr<Image> source, int tile_width, int tile_height)
...
bool TilesetImageWrapper::redraw()
{
  ...
  if (redraw_mask.not_set) return false;
  ...
  for (const auto &idx_tile : tilemap->tiles)
  {
    ...
    TilesetImageWrapper::painter.set_target(image);
    tileset->draw_tile(tile, x_offset, y_offset, TilesetImageWrapper::painter);
  }
