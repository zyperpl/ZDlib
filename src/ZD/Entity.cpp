...
namespace ZD
{
  ...
  glm::mat4 Entity::get_model_matrix() const
  {
    glm::mat4 translate_mat = glm::translate(
      glm::mat4(1), glm::vec3(position.x, position.y, position.z));
    glm::mat4 rotate_mat =
  ...
  void Entity::draw(const ShaderProgram &shader_program, const View &view)
  {
    ...
    assert(v_location);
    glUniformMatrix4fv(
      v_location->location, 1, GL_FALSE, glm::value_ptr(view_matrix));

    auto m_location = shader_program.get_uniform("M");
    assert(m_location);
    glUniformMatrix4fv(
      m_location->location, 1, GL_FALSE, glm::value_ptr(model_matrix));
